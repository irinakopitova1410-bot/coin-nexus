...
def render_cashflow_analysis():
    ...
    # ── Input dati ────────────────────────────────────────────────────────────
    with st.expander("📋 Inserisci / Modifica dati", expanded=not bool(erp_data)):
        company_name = st.text_input("Nome Azienda", value=company_name or "", key="cf_company")
    ...
    col3.metric("FCF Margin", f"{cf.fcf_margin:.1%}",
                "vs revenue")
    col4.metric("Cash Conversion", f"{cf.cash_conversion_ratio:.2f}x",
                "OCF/Utile Netto")
    ...
    fig_wf.update_layout(height=420, title="Cash Flow Waterfall (€)",
                          showlegend=False)
