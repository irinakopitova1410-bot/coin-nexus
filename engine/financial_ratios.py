...
def _status(value, good_min=None, good_max=None, bad_min=None, bad_max=None,
    ...
    """Determina lo stato di un ratio."""
    if value is None:
        return "warning"
    if higher_is_better:
...
def calculate_all_ratios(data: Dict[str, float], industry: str = "Manifatturiero") -> FinancialRatiosResult:
    """
    Calcola tutti i ratio finanziari dai dati in input.
    data: dizionario con chiavi standard NEXUS (total_assets, revenue, ebit, ecc.)
    """
    bm = INDUSTRY_BENCHMARKS.get(industry, INDUSTRY_BENCHMARKS["Manifatturiero"])
    alerts = []
    strengths = []

    # Estrai valori con default 0
    total_assets = data.get("total_assets", 0)
    total_liabilities = data.get("total_liabilities", 0)
    revenue = data.get("revenue", 0)
    ebit = data.get("ebit", 0)
    ebitda = data.get("ebitda", ebit * 1.15)  # stima se mancante
    ...
    liq_ratios = [
        ...
        Ratio("Cash Ratio", cash_ratio or 0, bm.get("cash_ratio"),
              ...
              "Solo liquidità immediata vs passività correnti",
              "Disponibilità Liquide / Passivo Corrente", "x"),
    ]
    if current_ratio and current_ratio < 1.0:
        alerts.append("⚠️ Current Ratio < 1: rischio liquidità a breve termine")
    if current_ratio and current_ratio > 2.5:
        strengths.append("✅ Eccellente liquidità corrente")

    cat_liq = RatioCategory("Liquidità", "💧", liq_ratios)
    ...
    net_debt_ebitda = _safe_div(total_debt - current_assets * 0.2, ebitda) if ebitda > 0 else None
    ...
    solid_ratios = [
        ...
        Ratio("Debt/Assets", debt_assets or 0, bm.get("debt_assets"),
              ...
              "Percentuale delle attività finanziata da debito",
    ...
    cat_solid = RatioCategory("Solidità Patrimoniale", "🏦", solid_ratios)
    ...
    gross_margin = data.get("gross_margin_raw", None)  # se disponibile
    ...
    red_ratios = [
        ...
        Ratio("ROA", roa or 0, bm.get("roa"),
              ...
              "Rendimento del totale delle attività",
              "Utile Netto / Totale Attivo", "%"),
        Ratio("ROI (EBIT/Assets)", roi or 0, bm.get("roi"),
        ...
        Ratio("EBITDA Margin", ebitda_margin or 0, bm.get("ebitda_margin"),
              ...
              "Redditività operativa lorda rispetto ai ricavi",
              "EBITDA / Ricavi", "%"),
    ...
    if roe and roe < 0:
        alerts.append("🔴 ROE negativo: l'azienda sta distruggendo valore per gli azionisti")
    if ebitda_margin and ebitda_margin > 0.20:
        strengths.append("✅ EBITDA Margin eccellente (>20%)")

    cat_red = RatioCategory("Redditività", "📈", red_ratios)
    ...
    eff_ratios = [
        Ratio("Asset Turnover", asset_turnover or 0, bm.get("asset_turnover"),
              ...
              "Quante volte le attività si 'girano' in ricavi",
    ...
    if dso is not None:
        eff_ratios.append(Ratio("DSO — Giorni Credito", dso, bm.get("dso"),
                                ...
                                "(Crediti Commerciali / Ricavi) × 365", "giorni"))
    if dpo is not None:
        eff_ratios.append(Ratio("DPO — Giorni Debito", dpo, bm.get("dpo"),
                                ...
                                "(Debiti Commerciali / Ricavi) × 365", "giorni"))
    if dio is not None:
        eff_ratios.append(Ratio("DIO — Giorni Magazzino", dio, bm.get("dio"),
                                ...
                                "(Magazzino / Ricavi) × 365", "giorni"))
    if ccc is not None:
        eff_ratios.append(Ratio("CCC — Cash Conversion Cycle", ccc, None,
                                _status(ccc, None, 60, None, 120, higher_is_better=False),
                                "Ciclo di conversione della cassa (DSO + DIO − DPO)",
                                "DSO + DIO − DPO", "giorni"))
        if ccc > 90:
            alerts.append(f"⚠️ Cash Conversion Cycle elevato ({ccc:.0f} gg): capitale circolante assorbito")

    if dso and dso > bm.get("dso", 60) * 1.3:
        alerts.append(f"⚠️ DSO {dso:.0f} gg superiore alla media settore: problema incasso crediti")

    cat_eff = RatioCategory("Efficienza Operativa", "⚙️", eff_ratios)

    # ─── 5. COPERTURA E FLUSSI ───────────────────────────────────────────────
    # Stima operating cash flow se non disponibile
    ...
    debt_service = _safe_div(ebitda, (interest_expense + data.get("debt_repayment", 0))) if (interest_expense + data.get("debt_repayment", 0)) > 0 else None

    cov_ratios = [
        Ratio("EBITDA / Totale Debiti", _safe_div(ebitda, total_debt) or 0, None,
              _status(_safe_div(ebitda, total_debt), 0.25, None, 0.10),
              "Capacità di ripagare il debito con la gestione operativa",
    ...
    if cashflow_coverage is not None:
        cov_ratios.append(Ratio("Cash Flow Coverage", cashflow_coverage, None,
                                ...
                                "Capacità del cash flow di coprire il debito",
                                "Cash Flow Operativo / Debiti Finanziari", "x"))

    cat_cov = RatioCategory("Copertura e Flussi", "💰", cov_ratios)

    # ─── 6. CRESCITA (se dati storici disponibili) ───────────────────────────
    growth_ratios = []
    ...
    if rev_growth is not None:
        growth_ratios.append(Ratio("Crescita Ricavi YoY", rev_growth, 0.05,
                                   ...
                                   "(Ricavi_t − Ricavi_{t-1}) / Ricavi_{t-1}", "%"))
    ...
    cat_growth = RatioCategory("Crescita", "🚀", growth_ratios)
