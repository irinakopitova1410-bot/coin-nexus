...
st.sidebar.image("https://tuo-logo-url.com/logo.png", width=200) # Opzionale
...
if uploaded_file and nome_azienda:
            ...
            with st.status("Invio dati al server Nexus AI...", expanded=True) as status:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        completato = False
                        tentativi = 0
                        max_tentativi = 12 # 12 * 5 secondi = 1 minuto di attesa max
                        
                        while not completato and tentativi < max_tentativi:
                            time.sleep(5) 
                            # Interroga Supabase per vedere se il worker ha finito
                        ...
                        if not completato:
                            st.warning("L'analisi sta richiedendo più tempo del previsto. Controlla la tua dashboard Supabase tra poco.")
    ...
    st.info("Inserisci il nome azienda e carica un file per iniziare.")

---
st.caption("Powered by Nexus AI Engine 2026 - Versione Enterprise")
