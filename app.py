...
# --- CONNESSIONE SUPABASE (DASHBOARD) ---
@st.cache_resource
def init_supabase():
    try:
        # Prende i dati dai Secrets di Streamlit
...
if st.button("Esegui Analisi API"):
    if company and revenue > 0:
        with st.spinner("Interrogazione motore Nexus in corso..."):
            try:
                # Chiamata al tuo backend su Render
                # Cambia l'URL se quello di Render è diverso
        ...
        st.warning("Inserisci tutti i dati minimi.")
...
if st.session_state.auth_user['role'] == "admin":
    ...
    if supabase_client:
        ...
        st.warning("⚠️ Configurazione Supabase mancante nei Secrets di Streamlit.")
